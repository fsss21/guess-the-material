...
class ServerSetup {
  constructor() {
    ...
    if (this.isPkg) {
      // В pkg режиме: данные в build/data/materials.json (рядом с launch.exe)
      this.dataFile = path.join(this.baseDir, 'data', this.config.dataFileName);
      this.dataFileFallback = null;
    } else {
      // В режиме разработки: пробуем build/ (после сборки), затем public/ (исходники)
      const buildPath = path.join(this.buildDir, this.config.dataFileName);
      const publicPath = path.join(this.baseDir, 'public', this.config.dataFileName);
